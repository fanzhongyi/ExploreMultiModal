...
class NLVR2Dataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        while result is None:
            try:
                image_tensor_0 = self.get_image(index,
                                                image_key="image_0")["image"]
                image_tensor_1 = self.get_image(index,
                                                image_key="image_1")["image"]
                text = self.get_text(index)["text"]
                result = True
            except:
                print(
                    f"error read file idx {index} in {self.names[0]}",
                    file=sys.stderr,
                )
